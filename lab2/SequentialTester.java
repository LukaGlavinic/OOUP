...
public class SequentialTester implements DistributionTester{
    ...
    @Override
    public int percentilRank(int p, ArrayList<Integer> listaBrojeva) {
    	Collections.sort(listaBrojeva);
        int np = (int) Math.ceil(p * listaBrojeva.size() / 100);
        return listaBrojeva.get(np - 1);
    }
